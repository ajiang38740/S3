...
function print_debug(msg, value, printLineAbove, printLineBelow) {
...
function objectPutACL(authInfo, request, log, cb) {
    ...
    print_debug('got to put object acl', '', true);
    ...
    return async.waterfall([
        function validateBucketAndObj(next) {
            print_debug('got to metadataValidateBucketAndObj', '', true);
            print_debug('metadataValParams:', metadataValParams);
            return metadataValidateBucketAndObj(metadataValParams, log,
                (err, bucket, objectMD) => {
                    print_debug('err:', err);
                    print_debug('result bucket:', bucket);
                    print_debug('result objMD:', objectMD, false, true);
                    if (err) {
                        return next(err);
                    }
                    if (!objectMD) {
        ...
        function parseAclFromXml(bucket, objectMD, next) {
            print_debug('got to parseAclFromXml', true);
            ...
            if (newCannedACL === undefined
                ...
                print_debug('using xml, request.post:', request.post);
                if (request.post) {
            ...
            */
            print_debug('using acl from requestHeaders:', request.headers);
        ...
        function processAcls(bucket, objectMD, jsonGrants, aclOwnerID, next) {
            print_debug('got to processAcls', '', true);
            if (newCannedACL) {
            ...
            print_debug('aclOwnerID (from xml):', aclOwnerID);
            print_debug('owner ID from objectMD:', objectMD['owner-id']);
            if (aclOwnerID && (aclOwnerID !== objectMD['owner-id'])) {
                print_debug('incorrect owner ID provided in ACL, ' +
            ...
            print_debug('no aclOwnerID or it matches objMD owner id', '');
            /**
            ...
            */
            if (jsonGrants) {
                print_debug('processing jsonGrants:', jsonGrants, true);
                ...
                print_debug('looping through access headers', '', true);
                print_debug('grantReadHeader', grantReadHeader);
                print_debug('grantReadACPHeader', grantReadACPHeader);
                print_debug('grantWriteACPHeader', grantWriteACPHeader);
                print_debug('grantFullControlHeader', grantFullControlHeader,
                false, true);
            ...
            print_debug('processing usersIdentifiedByEmail:',
            usersIdentifiedByEmail, true);
            const justEmails = usersIdentifiedByEmail
                .map(item => item.identifier);
            // If have to lookup canonicalID's do that asynchronously
            if (justEmails.length > 0) {
                print_debug('going to vault to get canonical IDs (justEmails):',
                justEmails);
                return vault.getCanonicalIds(
                    justEmails, log, (err, results) => {
                        print_debug('err from vault:', err);
                        print_debug('results from vault:', results, false,
                        true);
                        if (err) {
                        ...
                        print_debug('reconstructedUsersIdentifiedByEmail:',
                            reconstructedUsersIdentifiedByEmail);
                        print_debug('usersIdentifiedByID', usersIdentifiedByID);
                        print_debug('usersIdentifiedByGroup',
                        usersIdentifiedByGroup, false, true);
        ...
        function addAclsToObjMD(bucket, objectMD, ACLParams, next) {
            ...
            print_debug('about to add acls to object md', '', true, true);
            acl.addObjectACL(bucket, objectKey, objectMD,
                ACLParams, params, log, err => next(err, bucket, objectMD));
        },
    ], (err, bucket, objectMD) => {
        print_debug('got to waterfall callback', '', true);
        print_debug('err in waterfall cb:', err);
        ...
        print_debug('about to return to routes with:', resHeaders, false, true);
