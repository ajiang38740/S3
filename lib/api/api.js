...
const api = {
    callApiMethod(apiMethod, request, response, log, callback) {
        print_debug('got to callApiMethod', '', true);
        print_debug('apiMethod:', apiMethod);
        let returnTagCount = true;

        print_debug('validating query and headers', '', true);
        const validationRes =
            validateQueryAndHeaders(request.method, request.query,
                request.headers, log);
        print_debug('validationRes:', validationRes);
        if (validationRes.error) {
            print_debug('about to return error', '');
            print_debug('request.method:', request.method);
            print_debug('request.query:', request.query);
            print_debug('request.headers:', request.headers, '', false, true);
        ...
        print_debug('about to doAuth', '', true);
        print_debug('requestContexts:', requestContexts);
        return auth.server.doAuth(request, log, (err, userInfo,
            authorizationResults, streamingV4Params) => {
            print_debug('err from doAuth:', err);
            print_debug('userInfo from doAuth:', userInfo);
            print_debug('authorizationResults:', authorizationResults);
            print_debug('streamingV4Params:', streamingV4Params);
            if (err) {
                ...
                if (apiMethod === 'objectGet') {
                    ...
                    for (let i = 0; i < authorizationResults.length; i++) {
                        if (!authorizationResults[i].isAllowed) {
                            print_debug('about to return accessDenied', '');
                            print_debug('denial result in auth results:',
                            authorizationResults[i]);
            ...
            print_debug('doAuth succeeded, continuing to api', '', false, true);
